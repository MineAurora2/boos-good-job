...
点击“修改”时写入当前输入；输入事件更新草稿；“取消”删除草稿并重绘；保存期间记录账号级 pending 值并禁用编辑控件；响应只在 pending 值仍匹配时结算，成功且草稿未改变才删除草稿，失败或新草稿则保留草稿。
...
Run: `node --test --test-isolation=none --test-name-pattern="account quota save settles|account quota editor disables" tests/test_dashboard_contract.js`

Expected: 进行中的保存禁用编辑控件；旧响应不会清理更新后的草稿或 pending 请求。
...
Expected: 已确认 Python 97/97、userscript 43/43 和配额聚焦测试通过；Dashboard 契约 22/25 通过，剩余 3 项是本次修改前已存在的生命周期/旧指标断言失配。
